import numpy as np
import pandas as pd
import xgboost as xgb
import itertools
import data_export

from sklearn.metrics import mean_absolute_error
from sklearn.model_selection import KFold
from scipy.stats import skew, boxcox
from sklearn import preprocessing
from sklearn.preprocessing import StandardScaler
from time import time


shift = 200
n_folds = 10
COMB_FEATURE = 'cat80,cat87,cat57,cat12,cat79,cat10,cat7,cat89,cat2,cat72,' \
               'cat81,cat11,cat1,cat13,cat9,cat3,cat16,cat90,cat23,cat36,' \
               'cat73,cat103,cat40,cat28,cat111,cat6,cat76,cat50,cat5,' \
               'cat4,cat14,cat38,cat24,cat82,cat25'.split(',')


def encode(char_code):
    r = 0
    ln = len(str(char_code))
    for cc in range(ln):
        r += (ord(str(char_code)[cc]) - ord('A') + 1) * 26 ** (ln - cc - 1)
    return r


fair_constant = 0.7


def fair_obj(predicted, actual):
    labels = actual.get_label()
    xx = (predicted - labels)
    den = abs(xx) + fair_constant
    grad = fair_constant * xx / den
    hess = fair_constant * fair_constant / (den * den)
    return grad, hess


def xg_eval_mae(predicted, actual):
    y = actual.get_label()
    return 'mae', mean_absolute_error(np.exp(y) - shift, np.exp(predicted) - shift)


def mungeskewed(train, test, numeric_feats):
    ntrain = train.shape[0]
    test['loss'] = 0
    tt = pd.DataFrame(pd.concat((train, test))).reset_index(drop=True)
    skewed_feats = train[numeric_feats].apply(lambda a: skew(a.dropna()))
    skewed_feats = skewed_feats[skewed_feats > 0.25]
    skewed_feats = skewed_feats.index

    for feats in skewed_feats:
        tt[feats] += 1
        tt[feats], lam = boxcox(tt[feats])
    return tt, ntrain


def prepare_data():
    """
    Run this code once and save modified data to a file
    """
    print('\nStarted')
    directory = '../data/'
    train = pd.read_csv(directory + 'train.csv')
    test = pd.read_csv(directory + 'test.csv')

    loss = train['loss'].values
    id_train = train['id'].values
    id_test = test['id'].values

    print 'Data have been loaded'
    numeric_feats = [x for x in train.columns[1:-1] if 'cont' in x]
    categorical_feats = [x for x in train.columns[1:-1] if 'cat' in x]
    train_test, ntrain = mungeskewed(train, test, numeric_feats)

    print "Vladimir's part"
    # taken from Vladimir's script (https://www.kaggle.com/iglovikov/allstate-claims-severity/xgb-1114)
    for column in list(train.select_dtypes(include=['object']).columns):
        if train[column].nunique() != test[column].nunique():
            set_train = set(train[column].unique())
            set_test = set(test[column].unique())
            remove_train = set_train - set_test
            remove_test = set_test - set_train

            remove = remove_train.union(remove_test)

            def filter_cat(var):
                if var in remove:
                    return np.nan
                return var

            train_test[column] = train_test[column].apply(lambda var: filter_cat(var), 1)

    # taken from Ali's script (https://www.kaggle.com/aliajouz/allstate-claims-severity/singel-model-lb-1117)
    train_test["cont1"] = np.sqrt(preprocessing.minmax_scale(train_test["cont1"]))
    train_test["cont4"] = np.sqrt(preprocessing.minmax_scale(train_test["cont4"]))
    train_test["cont5"] = np.sqrt(preprocessing.minmax_scale(train_test["cont5"]))
    train_test["cont8"] = np.sqrt(preprocessing.minmax_scale(train_test["cont8"]))
    train_test["cont10"] = np.sqrt(preprocessing.minmax_scale(train_test["cont10"]))
    train_test["cont11"] = np.sqrt(preprocessing.minmax_scale(train_test["cont11"]))
    train_test["cont12"] = np.sqrt(preprocessing.minmax_scale(train_test["cont12"]))

    train_test["cont6"] = np.log(preprocessing.minmax_scale(train_test["cont6"]) + 0000.1)
    train_test["cont7"] = np.log(preprocessing.minmax_scale(train_test["cont7"]) + 0000.1)
    train_test["cont9"] = np.log(preprocessing.minmax_scale(train_test["cont9"]) + 0000.1)
    train_test["cont13"] = np.log(preprocessing.minmax_scale(train_test["cont13"]) + 0000.1)
    train_test["cont14"] = (np.maximum(train_test["cont14"] - 0.179722, 0) / 0.665122) ** 0.25

    print('')
    for comb in itertools.combinations(COMB_FEATURE, 2):
        feat = comb[0] + "_" + comb[1]
        train_test[feat] = train_test[comb[0]] + train_test[comb[1]]
        train_test[feat] = train_test[feat].apply(encode)
        print('Combining Columns:', feat)

    print('')
    for col in categorical_feats:
        print('Analyzing Column:', col)
        train_test[col] = train_test[col].apply(encode)

    # print(train_test[categorical_feats])

    # why do we apply StandardScaler after transformation above?
    # to normalize to mean=0 and sigma=1???
    ss = StandardScaler()
    train_test[numeric_feats] = ss.fit_transform(train_test[numeric_feats].values)

    dt_train = train_test.iloc[:ntrain, :].copy()
    dt_test = train_test.iloc[ntrain:, :].copy()

    print 'Shape of train data set: {}'.format(dt_train.shape)
    print 'Shape of test data set: {}'.format(dt_test.shape)

    print '='*40
    print dt_test
    print '=' * 40

    print 'write data to disk'

    dt_train['id'] = id_train
    dt_train['loss'] = loss
    dt_train.to_pickle('../data5/data-train')

    dt_test['id'] = id_test
    dt_test.to_pickle('../data5/data-test')


def get_train_data():
    ddt = pd.read_pickle('../data5/data-train')
    data_id = ddt['id'].values
    data_loss = ddt['loss'].values
    ddt = ddt.drop(['id', 'loss'], axis=1)
    return data_id, data_loss, ddt.values


def get_test_data():
    ddt = pd.read_pickle('../data5/data-test')
    data_id = ddt['id'].values
    ddt = ddt.drop(['id', 'loss'], axis=1)
    return data_id, ddt.values


def train_the_model():
    print 'Load training data'
    train_id, train_loss, train_data = get_train_data()

    train_loss = np.log(train_loss + shift)

    #
    xgb_rounds = []

    kf = KFold(n_splits=n_folds).split(train_data)
    for i, (train_index, test_index) in enumerate(kf):
        t0 = time()
        print('\n ==================== Fold %d ================' % (i + 1))
        #

        # x_train, x_val = train_data[train_index, :], train_data[test_index, :]
        y_train, y_val = train_loss[train_index], train_loss[test_index]

        params = {
            'seed': np.random.randint(np.iinfo(np.int32).max),
            'colsample_bytree': 0.7,
            'silent': 1,
            'subsample': 0.7,
            'learning_rate': 0.03,
            'objective': 'reg:linear',
            'max_depth': 12,
            'min_child_weight': 100,
            'booster': 'gbtree'}

        print 'Create DMatrix'
        d_train = xgb.DMatrix(train_data[train_index, :], label=y_train)
        d_valid = xgb.DMatrix(train_data[test_index, :], label=y_val)
        watchlist = [(d_train, 'train'), (d_valid, 'eval')]

        print 'Train the model'
        clf = xgb.train(params,
                        d_train,
                        100000,
                        watchlist,
                        early_stopping_rounds=50,
                        obj=fair_obj,
                        feval=xg_eval_mae)

        xgb_rounds.append(clf.best_iteration)
        predict_val = clf.predict(d_valid, ntree_limit=clf.best_ntree_limit)
        cv_score = mean_absolute_error(np.exp(y_val)-shift, np.exp(predict_val)-shift)
        print('eval-MAE: %.6f' % cv_score)

        print 'Save trained model'
        clf.save_model('../xgb_models/model-{}'.format(i+36))

        print 'One iteration takes {} minutes'.format((time() - t0) / 60.0)

    #
    print xgb_rounds


def test_the_model(sim_n, n_folds_test, threshold=2000):
    test_id, test_data = get_test_data()
    predict_loss = np.zeros(test_id.shape[0], dtype=float)
    d_test = xgb.DMatrix(test_data)

    #
    average_counter = 0
    for i in range(n_folds_test):

        clf = xgb.Booster()
        clf.load_model('../xgb_models/model-{}'.format(i))

        print 'Iteration: {}    best score: {}'.format(i, clf.attributes()['best_score'])
        # print clf.attributes()
        if float(clf.attributes()['best_score']) < threshold:
            p = clf.predict(d_test)
            predict_loss += (np.exp(p) - shift)
            average_counter += 1

    predict_loss /= float(average_counter)
    data_export.save_submission_xgb(test_id, predict_loss, 'xgb-{}-avg'.format(sim_n))
    print 'Perform average of {} simulation out of {} with threshold {}'.format(average_counter,
                                                                                n_folds_test,
                                                                                threshold)

if __name__ == "__main__":

    print 'Prepare the data'

    # train_the_model()

    test_the_model(sim_n=43,
                   n_folds_test=46,
                   threshold=2000)
