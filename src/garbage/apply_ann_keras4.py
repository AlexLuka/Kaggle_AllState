...
predict_res2 = np.zeros(shape=(test_id.shape[0], 1), dtype=float)
ggg2 = 0
...
while ggg < n_sim:
    ...
    if mae_test < 1140.0:
        ...
        predict_res2 += np.exp(sd * predict_test + mu) - shift_out

        ggg2 += 1

    # print '='*80

predict_res /= float(ggg)
predict_res2 /= float(ggg2)

src.data_export.save_submission(test_id, predict_res, '{}-avg-less1140'.format(run))
src.data_export.save_submission(test_id, predict_res2, '{}-avg-more1140'.format(run))
